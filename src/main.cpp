...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod) {
    ...
    if (key == GLFW_KEY_Q && action == GLFW_PRESS) {
        // Rotação do braço superior esquerdo
        if (mod & GLFW_MOD_SHIFT)
            g_LeftUpperArmAngleX -= delta;
        else
            g_LeftUpperArmAngleX += delta;
    }

    if (key == GLFW_KEY_W && action == GLFW_PRESS) {
        // Rotação do antebraço esquerdo
        if (mod & GLFW_MOD_SHIFT)
            g_LeftForearmAngleX -= delta;
        else
            g_LeftForearmAngleX += delta;
    }

    if (key == GLFW_KEY_E && action == GLFW_PRESS) {
        // Rotação do braço superior direito
        if (mod & GLFW_MOD_SHIFT)
            g_RightUpperArmAngleX -= delta;
        else
            g_RightUpperArmAngleX += delta;
    }

    if (key == GLFW_KEY_R && action == GLFW_PRESS) {
        // Rotação do antebraço direito
        if (mod & GLFW_MOD_SHIFT)
            g_RightForearmAngleX -= delta;
        else
            g_RightForearmAngleX += delta;
    }
    ...
    if (key == GLFW_KEY_T && action == GLFW_PRESS) {
        // Rotação das mãos
        if (mod & GLFW_MOD_SHIFT) {
            g_LeftHandAngleX -= delta;
            g_RightHandAngleX -= delta;
        } else {
            g_LeftHandAngleX += delta;
            g_RightHandAngleX += delta;
        }
    ...
    if (key == GLFW_KEY_A && action == GLFW_PRESS) {
        // Rotação da coxa esquerda
        if (mod & GLFW_MOD_SHIFT)
            g_LeftUpperLegAngleX -= delta;
        else
            g_LeftUpperLegAngleX += delta;
    }

    if (key == GLFW_KEY_S && action == GLFW_PRESS) {
        // Rotação da canela esquerda
        if (mod & GLFW_MOD_SHIFT)
            g_LeftLowerLegAngleX -= delta;
        else
            g_LeftLowerLegAngleX += delta;
    }

    if (key == GLFW_KEY_D && action == GLFW_PRESS) {
        // Rotação da coxa direita
        if (mod & GLFW_MOD_SHIFT)
            g_RightUpperLegAngleX -= delta;
        else
            g_RightUpperLegAngleX += delta;
    }

    if (key == GLFW_KEY_F && action == GLFW_PRESS) {
        // Rotação da canela direita
        if (mod & GLFW_MOD_SHIFT)
            g_RightLowerLegAngleX -= delta;
        else
            g_RightLowerLegAngleX += delta;
    }
    ...
    if (key == GLFW_KEY_G && action == GLFW_PRESS) {
        // Rotação dos pés
        if (mod & GLFW_MOD_SHIFT) {
            g_LeftFootAngleX -= delta;
            g_RightFootAngleX -= delta;
        } else {
            g_LeftFootAngleX += delta;
            g_RightFootAngleX += delta;
        }
